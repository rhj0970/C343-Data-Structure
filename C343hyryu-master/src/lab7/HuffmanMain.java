...
class HuffTree<Key, E> {
	...
	public String encode(String instr) {
		...
		for (int i=0; i<instr.length(); i++) {
			String s = instr.substring(i, i+1); // from where to start and where to end
			code += codeTable.get(s); // we append that values to the empty string
			
		}
		instr = code; // Since the empty strings are filled, we make that to in instr and return it.
	...
	public String decode(String coded) {
		...
		String k = "";
		BinNode<Integer, String> temp = root;
		char ch;
